...
class WhisperXServer:
    """
    Constructor for WhisperXServer
    """
    def __init__(self, argsDict: dict):
        self.args_dict = argsDict

        self.model = AmadeoWhisperX(argsDict)
        self.server = AmadeoServer(argsDict['host'], argsDict['port'], additional_client_functionality = self.model.handle_client_request)
